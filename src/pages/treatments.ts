...
export function treatmentDetailPage(slug: string): { html: string; title: string; description: string; schemas: object[] } | null {
  ...
  const treatmentFAQs: Record<string, { q: string; a: string }[]> = {
    'implant': [
      ...
      { q: '임플란트 시술 기간은 얼마나 걸리나요?', a: '식립 수술은 30분~1시간, 뼈 유착 기간 2~4개월, 이후 CEREC 당일 보철 장착까지 총 3~5개월이 일반적입니다.' },
    ],
    'cerec': [
      { q: 'CEREC으로 어떤 보철을 만들 수 있나요?', a: '현재 강남치과의원에서는 CEREC으로 싱글 지르코니아 크라운과 싱글 세라믹 크라운을 제작하고 있습니다. 임플란트 보철, 인레이, 브릿지 등은 기공소를 통해 정밀 제작합니다.' },
