...
export function treatmentDetailPage(slug: string): { html: string; title: string; description: string; schemas: object[] } | null {
  ...
  const treatmentFAQs: Record<string, { q: string; a: string }[]> = {
    ...
    'cerec': [
      { q: 'CEREC 당일 보철이란 무엇인가요?', a: 'CEREC은 디지털 스캔 → AI 설계 → 밀링 → 소성까지 1~2시간 만에 세라믹 보철물을 제작하는 첨단 시스템입니다. 본뜨기 없이 당일 치료 완료가 가능합니다.' },
      { q: 'CEREC 보철은 내구성이 좋은가요?', a: '리튬 디실리케이트(e.max) 세라믹 소재로 10년 이상 사용 가능하며, 자연치아와 유사한 강도와 색감을 제공합니다.' },
      { q: 'CEREC으로 어떤 치료가 가능한가요?', a: '크라운, 인레이, 온레이, 라미네이트 등 대부분의 보철 치료가 가능합니다. 충치 치료 후 당일 보철 장착까지 완료됩니다.' },
      { q: '기존 보철과 CEREC의 차이점은 무엇인가요?', a: '기존 방식은 본뜨기→기공소 외주→2~3주 대기가 필요하지만, CEREC은 디지털 스캔→원내 제작→당일 장착으로 시간과 내원 횟수를 획기적으로 줄입니다.' },
    ],
    'invisalign': [
    ...
    'cosmetic': [
      ...
      { q: '심미 보철은 얼마나 자연스러운가요?', a: 'CEREC 올세라믹/지르코니아 보철은 자연치아와 거의 구분할 수 없는 색감과 투명도를 재현합니다. 금속 없는 보철로 잇몸 변색도 없습니다.' },
      { q: '심미 보철 치료 기간은 얼마나 걸리나요?', a: 'CEREC 시스템으로 당일 1~2시간 이내 완료 가능합니다. 별도의 기공소 외주가 필요 없어 추가 내원이 불필요합니다.' },
    ],
    'wisdom-tooth': [
    ...
    'cavity': [
      { q: '충치 치료는 하루 만에 끝나나요?', a: '초기~중기 충치는 CEREC 시스템으로 당일 1~2시간 내에 레진, 인레이, 크라운 치료까지 완료 가능합니다.' },
      { q: '충치가 심하면 어떤 치료를 받나요?', a: '충치가 신경까지 진행된 경우 신경치료(근관치료) 후 크라운으로 보호합니다. CEREC 당일 보철로 크라운 제작이 가능합니다.' },
    ...
    'root-canal': [
      ...
      { q: '신경치료 후에는 어떤 치료가 필요한가요?', a: '신경치료 후 치아가 약해지므로 크라운을 씌워 보호합니다. CEREC으로 당일 크라운 제작이 가능합니다.' },
      { q: '신경치료는 아프나요?', a: '국소마취로 치료 중 통증은 없습니다. 이미 통증이 심한 경우 신경치료를 통해 오히려 통증이 해소됩니다.' },
    ],
    'crown': [
      { q: '크라운 치료는 얼마나 걸리나요?', a: 'CEREC 시스템으로 스캔 5분 → 설계 10분 → 밀링 15분 → 소성 15분 → 장착 10분, 총 1~2시간이면 당일 완료됩니다.' },
